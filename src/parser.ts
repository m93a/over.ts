...
export function signatureToArgumentGuard(
    ...
    loop:
    for (let pos = 0; pos < signature.length;) {
        ...
        switch (state) {
            case ParserState.Start:
                ...
                else if (isComma(ch)) throw new SyntaxError(`Unexpected comma at position ${pos}.`);
            ...
            case ParserState.BeforeWord:
                ...
                else if (isArrow(ch, ch2)) throw new SyntaxError(`Unexpected trailing comma at position ${pos}.`);
                else if (isComma(ch)) throw new SyntaxError(`Duplicit comma at position ${pos}.`);
                else throw new SyntaxError(`Unexpected character at position ${pos}: ${ch}`);
                break;
